...
<div>
    <legend>{sprintf(lang('photos_legend_3'), count($photos), $tag)}
    ...
    <legend>{lang('photos_legend_4')}</legend>
    <div id="cloud">
    {foreach from=$tags item=tag}
        <a href="javascript:void(0);" onclick="{$url_index}/photos/tag/{$tag.name}" class="{$tag.class}">{$tag.name}</a>
    {/foreach}
    </div>
</div>
{/strip}
